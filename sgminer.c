...
static char *set_pool_algorithm(const char *arg)
{
  ...
  applog(LOG_DEBUG, "Setting pool %i algorithm to %s", pool->pool_no, "BCD");
  set_algorithm(&pool->algorithm);
...
bool test_nonce(struct work *work, uint32_t nonce)
{
  ...
  if (le32toh(*hash_32) > diff1targ) {
    applog(LOG_DEBUG, "test_nonce(struct work, uint32_t) going to return false??");
  }
...
int main(int argc, char *argv[])
{
  ...
  set_algorithm(&default_profile.algorithm);
